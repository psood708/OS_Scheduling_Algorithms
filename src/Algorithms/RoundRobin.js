...
class ProcessStruct {
    constructor(pid, at, bt, ct, wt, start_time, tat, rt, bt_remaining) {
        ...
        this.ct = ct; //complete time
        this.wt = wt; //waiting time
        this.tat = tat; //turnaround time
        this.rt = rt;  //let's see
...
//here we will initialize for the gantt charts that we are creating
let ganttCharPD = [];
let ganttCharAT = [];

//Making Array Of Objects
//here we'll be making the processes that we require for our computation
let ps = [];
...
btnAdd.addEventListener('click', function addRow(e) {
    ...
    //we are putting in the conditions that need to be satisfied in order to get this correct
